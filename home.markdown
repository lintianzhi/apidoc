...
七牛云存储的使用非常简单。根据不同的用况，用户可以选择适合自身需求的方式。
...
$ # Windows用户
$ qrsync.exe up_cfg.json
```

"up_cfg.json"是配置文件，包含上传所需的参数，用于控制文件上传，详见[qrsync使用说明](http://docs.qiniu.com/tools/qrsync.html)。qrsync工具可以用于整个目录内文件的上传，对于仅需使用七牛云存储进行数据备份的用于而言，是最佳的选择。
